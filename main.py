...
mwdurl = os.environ.get("MWD_URL")
rsiurl = os.environ.get("RSI_URL")
sendFileUrl = "".join(["https://api.telegram.org/bot", bot_token,"/sendDocument?chat_id=",group_id])
...
def mwd():
    ...
    for stock in screened_stocks_list:
        bot.send_message(chat_id=group_id,text=f'{stock} :: MWD Setup {datetime.datetime.now(tz=gettz("Asia/Kolkata"))}')
    print("Ending Else")
                    ...
                    for hour in hours:
                        for minute in minutes:
                            ...
                            if (schedule_time >= datetime.time(9,15)) & (schedule_time <= datetime.time(15,30)):
                                print("Inside For and Before Test mwd execution ", datetime.datetime.now(tz=gettz("Asia/Kolkata")))
                                mwd()
